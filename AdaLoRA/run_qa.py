...
from adalora_trainer import Custom_QuestionAnsweringTrainer as QuestionAnsweringTrainer
...
from deberta_v2.modeling_deberta_v2 import DebertaV2ForQuestionAnswering
...
def main():
    ...
    # )
    metric = evaluate.load("./evaluate/metrics/squad")
    ...
    if training_args.do_train:
        ...
        print("rankallocator.rank_pattern", rankallocator.rank_pattern)
        sum_param = 0
        for n in rankallocator.rank_pattern:
            sum_param = sum_param + rankallocator.rank_pattern[n]
            print(f"Layer {n}: ranknum:{rankallocator.rank_pattern[n]}")

        print(f"Total layer num: {len(rankallocator.rank_pattern)}, Sum parameter:{sum_param}")
