...
from adalora_trainer import Custom_Trainer as Trainer
...
from deberta_v2.modeling_deberta_v2 import DebertaV2ForSequenceClassification
from deberta_v2.configuration_deberta_v2 import DebertaV2Config
...
class ModelArguments:
    ...
    reg_orth_coef: Optional[float] = field(
        default=0.0,
        metadata={"help": "Regularization Orthogonal Coefficient"},
    )
    ...
    target_rank: Optional[int] = field(
        default=16,
        metadata={"help": "Target average rank"},
    )
    target_total_rank: Optional[int] = field(
        default=None,
        metadata={"help": "Specify the total target rank"},
    )
    init_warmup: Optional[int] = field(
        default=4500,
        metadata={"help": "The total step of initial warmup"},
    )
    final_warmup: Optional[int] = field(
        default=12000,
        metadata={"help": "The total step of final fine-tuning"},
    )
...
def main():
    ...
    if data_args.task_name is not None:
        metric = evaluate.load("./evaluate/metrics/glue", data_args.task_name, cache_dir=model_args.cache_dir)
    elif is_regression:
        metric = evaluate.load("./evaluate/metrics/mse", cache_dir=model_args.cache_dir)
    else:
        # metric = evaluate.load("./evaluate/metrics/accuracy", cache_dir=model_args.cache_dir)
        metric = evaluate.load("./evaluate/metrics/glue", data_args.dataset_name.split('/')[-1],
                               cache_dir=model_args.cache_dir)
    ...
    if training_args.do_train:
        ...
        if model_args.apply_adalora and model_args.lora_type=='svd':

            txt_output_file = os.path.join(training_args.output_dir, 'rank_num.txt')

            with open(txt_output_file, 'w') as f:

                sum_param = 0
                for n in rankallocator.rank_pattern:
                    sum_param += rankallocator.rank_pattern[n]
                    f.write(f"Layer {n}: ranknum:{rankallocator.rank_pattern[n]}\n")

                f.write(f"Total layer num: {len(rankallocator.rank_pattern)}, Sum parameter:{sum_param}\n")
