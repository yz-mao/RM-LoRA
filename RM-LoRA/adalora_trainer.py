...
class Custom_Trainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        print("I'm here in AdaTrainer's inner training loop!!!")
                            ...
                            if param.requires_grad:
                                ...
                                if param.grad is not None:
                                    ...
                                    if "lora_A" in name:
                                        ...
                                        selected_cols_A = random.sample(range(param.grad.size(0)), self.model_args.random_rank)
                                        mask[selected_cols_A, :] = 1

                                    elif "lora_B" in name:

                                        if self.model_args.lora_type=='v1':
                                            mask[:, selected_cols_A] = 1

                                        elif self.model_args.lora_type=='v2':
                                            selected_cols_B = random.sample(range(param.grad.size(1)), self.model_args.random_rank)
                                            mask[:, selected_cols_B] = 1
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Call the parent class's compute_loss method to calculate the base loss
        loss, outputs = super().compute_loss(model, inputs, return_outputs=True)

        # Add regularization loss if applicable
        if self.model_args.lora_type == 'svd':
            regu_loss = self.compute_svd_orth_regu(model)
        elif self.model_args.lora_type == 'frd':
            regu_loss = self.compute_frd_orth_regu(model)
        else:
            # regu_loss = self.compute_orth_regu(model)
            regu_loss = self.compute_svd_orth_regu(model)
    ...
    def compute_orth_regu(self, model):
        # The function to compute orthogonal regularization for LORALayer
        regu_loss = None
        num_param = 0

        for n, p in model.named_parameters():
            if "lora_A" in n or "lora_B" in n:
                para_cov = p.T @ p if "lora_A" in n else p @ p.T
                I = torch.eye(*para_cov.size(), out=torch.empty_like(para_cov))
                I.requires_grad = False
                num_param += 1
                if regu_loss is None:
